...
MiClouds : MultiOutUGen {
	...
	*ar {
		...
		record_reset=0, record_mode=0, phasor_reset=0, mul=1.0, add=0.0;
		^this.multiNewList(['audio', pit, pos, size, dens, tex, drywet, in_gain, spread, rvb, fb,
			freeze, mode, lofi, trig, record, speed, ssize, record_reset, record_mode, phasor_reset]
			++ inputArray.asArray).madd(mul);
	}

	checkInputs {
		if ( inputs.at(20).rate != 'audio', {
			^("input is not audio rate:" + inputs.at(20) + inputs.at(20).rate);
		});
